...
ACTIONS_RUNNER_INPUT_TOKEN="$(curl -sS --request POST --url "https://${GITHUB_API_BASE}/actions/runners/registration-token" --header "authorization: Bearer ${GITHUB_AUTH_TOKEN}"  --header 'content-type: application/json' | jq -r .token)"
# configure runner
RUNNER_ALLOW_RUNASROOT=1 /runner/config.sh --unattended --replace --work "/runner-tmp" --url "$REPO_URL" --token "$ACTIONS_RUNNER_INPUT_TOKEN" --labels gcp
