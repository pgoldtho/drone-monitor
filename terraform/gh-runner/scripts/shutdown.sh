...
RUNNER_ALLOW_RUNASROOT=1  /runner/config.sh remove --unattended --token "$(curl -sS --request POST --url "https://${GITHUB_API_BASE}/actions/runners/remove-token" --header "authorization: Bearer ${GITHUB_AUTH_TOKEN}"  --header "content-type: application/json" | jq -r .token)"